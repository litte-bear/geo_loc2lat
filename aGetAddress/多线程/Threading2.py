...
def GPS_to_Gcj(location):
    ...
    if data["status"] == "1":
        ...
        return None
def get_location_POI(index,address,city):
    try:
        parameters = {
            "key": key,
            "address": address,
...
with ThreadPoolExecutor(max_workers=max_workers) as executor:
    # 提交所有任务到线程池
    futures = [executor.submit(get_location_POI, i,address, city) for i,(address, city) in enumerate(zip(combined_addresses, citys))]
