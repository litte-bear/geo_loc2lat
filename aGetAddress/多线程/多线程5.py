...
def GPS_to_Gcj(location):
    ...
    if data["status"] == "1":
        ...
        return None
def get_location_POI(address,city):
    parameters = {
        "key": key,
        "address": address,
        "city": city,
...
file_path = '../datacut/info_12.csv'

df = pd.read_csv(file_path)
df = pd.DataFrame(df)

combined_addresses = df["地址"].to_numpy()
citys = df["中国申请人地市"].to_numpy()

data_group = []

for i,data in enumerate(combined_addresses):
    city = citys[i]
    geocodes = get_location_POI(data,city)
    # print(geocodes)
    if geocodes is not None and len(geocodes) > 0:
        first_geocode = [geocodes[0]]
        # print(first_geocode)
    else:
        first_geocode = [{'location':'0,0'}]
    data_group.append(first_geocode)
    print(f'查询地理编码中（进度：{i + 1}/{len(combined_addresses)}）...')
print('地理编码查询完成')
...
df.to_excel('../结果数据/output_file12.xlsx', index=False)
df.to_csv('../结果数据/output_file12.csv', index=False)
